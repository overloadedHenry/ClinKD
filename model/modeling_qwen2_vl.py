            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    if last_segment_type is None:
                        ...
                        st_idx_text = llm_pos_ids_list[-1].max().item() + (2 if last_segment_type != 'text' else 1)
                    text_pos = torch.arange(text_len, device=input_ids.device).view(1, -1).expand(3, -1) + st_idx_text
                    ...
                    st_idx_visual = llm_pos_ids_list[-1].max().item() + 2
                    visual_pos = torch.stack([t_index, h_index, w_index]) + st_idx_visual
                    llm_pos_ids_list.append(visual_pos)
                    last_segment_type = 'visual'
                ...
                if st < len(input_tokens):
                    st_idx_text = llm_pos_ids_list[-1].max().item() + 2
